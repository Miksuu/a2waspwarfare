...
Private ["_clients", "_defence", "_groups", "_moveInGunner", "_positions", "_side", "_team", "_town", "_townDefenderAI"];
...
// Marty: Optional town owner is passed to the HC so static gunners can be owner-marked there.
...
for '_i' from 0 to count(_groups) -1 do {
	...
	if (count _clients > 0) then {
		[leader(_clients select floor(random count _clients)), "HandleSpecial", ['delegate-ai-static-defence', _side, [_groups select _i], [_positions select _i], _team, _defence, _moveInGunner, _townDefenderAI, _town]] Call WFBE_CO_FNC_SendToClient;
	};
};
