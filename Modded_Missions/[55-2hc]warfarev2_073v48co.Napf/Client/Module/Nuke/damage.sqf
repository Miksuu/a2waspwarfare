...
_range = 500 ; //destruction nuke range around target.
_all_objects_in_range = nearestObjects [_target,[], 800]; // Marty : Do not use the nearObjects command! It's the [] that seems to do the trick : If I had declared any type there it would leave the trees/walls etc, even if ["All"] was used. This doesnt work with command nearObjects.
