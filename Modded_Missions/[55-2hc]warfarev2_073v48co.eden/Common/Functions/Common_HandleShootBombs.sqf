...
if (!(_ammo in ["Bo_FAB_250", "Bo_Mk82", "Bo_GBU12_LGB", "Bo_GBU12_LGB_F35"])) exitWith {}; // if ammo is not a bomb, we quit.
...
// Check for laser-guided bombs
if (_ammo in ["Bo_GBU12_LGB", "Bo_GBU12_LGB_F35"]) then {
    if (isNull cursorTarget || {!(cursorTarget isKindOf "LaserTarget")}) then {
        _vehicle vehicleChat localize "STR_WF_MESSAGE_BombNoLaserTarget";
