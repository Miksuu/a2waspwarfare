...
_objectsLimit = 100;								// Objects. Set -1 to disable.
_objectsDistCheck = FALSE;							// TRUE to delete any objects that are far from players.
_objectsDist = 300;								// Distance (meters) from players that objects are not deleted if below max.
_objectFilters = ["WeaponHolder", "GroundWeaponHolder", "WeaponHolderSimulated", "Mine", "MineE"];

_clutterLimit = 20;								// Clutters. Set -1 to disable.
_clutterDistCheck = TRUE;						// TRUE to delete any clutters that are far from players.
_clutterDist = 100;		
...
while {!cti_GameOver} do {
	...
	sleep 1;
	//=================================Clutter (the grass cutter object from bulldozer function)
	/* not needed anymore
	if (!(_clutterLimit isEqualTo -1)) then {
		if ((count (allMissionObjects "CUP_A1_ClutterCutter")) > _clutterLimit) then {
			while {(((count (allMissionObjects "CUP_A1_ClutterCutter")) - _clutterLimit) > 0)} do {
				deleteVehicle ((allMissionObjects "CUP_A1_ClutterCutter") select 0);
				sleep 0.5;
			};
		} else {
			if (_clutterDistCheck) then {
				{
					if ([_x,_clutterDist,player] call _isHidden) then {
						deleteVehicle _x;
					};
				} count (allMissionObjects "CUP_A1_ClutterCutter");
			};
		};
	};
	sleep 1;
