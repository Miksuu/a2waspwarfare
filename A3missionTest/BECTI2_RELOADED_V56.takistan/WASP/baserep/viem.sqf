SpecopscontainerBasehealth = [OSpecOps1,OSpecOps2,OSpecOps3,OSpecOps4,OSpecOps5,BSpecOps1,BSpecOps2,BSpecOps3,BSpecOps4,BSpecOps5];
...
{ 
	...
	/////// SNIPER
	if (player in SpecopscontainerBasehealth)then{
		_obj = cursortarget;
	...
	if (!isNull(commanderTeam)) then {if (commanderTeam == group player) then {_isCommander = true} else{_isCommander = false}};
