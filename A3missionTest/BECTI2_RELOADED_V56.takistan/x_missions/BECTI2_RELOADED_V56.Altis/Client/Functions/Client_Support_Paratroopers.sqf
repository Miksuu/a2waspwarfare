...
for '_i' from 1 to _vehicle_count do {
	...
	_vehicle = [missionNamespace getVariable Format ["cti_%1PARACARGO", _side], _spawnpos_plane ,_sideID, (_ranDir select _ran), true,true,true,"FLY"] Call cti_CO_FNC_CreateVehicle;
	...
	//changed to get marker on dedicated host
...
_paratroopers = [];
{
    if (_built_inf <= _vehicle_cargo) then {
        ...
        _unit = [_x,group player, [100,12000,0], _sideID] Call cti_CO_FNC_CreateUnit;
        
		//_unit = [_x, (_ranPos select _ran) ,_sideID, (_ranDir select _ran), true,true,true,"FORM"] Call cti_CO_FNC_CreateVehicle;
...
if (_greenlight) then {
	...
	while {true} do {
		...
		if (_vehicleCoord distance _positionCoord < 500) exitWith {};//--- Destination reached.
