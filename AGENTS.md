...
- When debugging mission logic on request, add `WF_Debug`-gated `WFBE_CO_FNC_LogContent` lines with the key values and decision path, so the relevant data lands in the RPT without spamming normal matches.
