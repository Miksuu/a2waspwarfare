...
if (_unit_kind in (missionNamespace getVariable "WFBE_REPAIRTRUCKS")) then { //--- Repair Trucks.
	...
	if ((missionNamespace getVariable "WFBE_C_GAMEPLAY_VICTORY_CONDITION") != 1) then { //--- Repair HQ Ability.
		...
		_unit addAction [format [localize 'STR_WF_Repair_MHQ %1', "[S", _repairPrice, "]"],'Client\Action\Action_RepairMHQ.sqf', [], 98, false, true, '', 'alive _target'];
	};
};
...
if (_isMan) then { //--- Man.
	...
	//
	if (_unit_kind in (missionNamespace getVariable Format['WFBE_%1SALVAGETRUCK',str _side])) then {_color = "ColorBlue";_type = "SalvageVehicle";};//--- Salvage.
