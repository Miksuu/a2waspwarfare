...
namespace A2WASPDiscordBot_Windows_App
{
    public class Player
    {
        ...
        {
            Log.WriteLine("Initializing new (?) player with GUID: " + id, LogLevel.DEBUG);
            this.id = id;
            ...
            Log.WriteLine("Exiting Player constructor method (GUID: " + id + ")", LogLevel.DEBUG);
        }
        ...
        public Boolean existsInDatabase(long guid)
        {
            try
            {
                using (var conn = new MySql.Data.MySqlClient.MySqlConnection())
                {
                    ...
                    {
                        Log.WriteLine(e.Number + ", " + e.Message, LogLevel.ERROR);
                        boolAsInt = 0;
                        conn.Close();
                        return false;
                    }
                    ...
                    if (boolAsInt > 0)
                    {
                        Log.WriteLine("Player with GUID: " + this.id + " was found from the database.", LogLevel.DEBUG);
                        return true;
                    }
                    else
                    {
                        Log.WriteLine("Player with GUID: " + this.id + " was not found from the database!", LogLevel.WARNING);
                        return false;
                    }
                }
            }
            ...
            {
                Log.WriteLine("Something went wrong when checking player's existence in database! Error code: " + e.Number + ": " + e.Message, LogLevel.ERROR);
                return false;
            }
        ...
        public long GetTotalTicks()
        {
            ...
            {
                Log.WriteLine(e.Number + ", " + e.ToString(), LogLevel.ERROR);
            }
